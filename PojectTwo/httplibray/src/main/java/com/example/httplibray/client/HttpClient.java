package com.example.httplibray.client;

import android.text.TextUtils;
import android.util.Log;


import com.example.httplibray.HttpConstant;
import com.example.httplibray.HttpGlobalConfig;
import com.example.httplibray.HttpManager;
import com.example.httplibray.api.ApiServer;
import com.example.httplibray.callback.BaseCallBack;
...
public class HttpClient {
    ...
    private void addHeadrs() {
        ...
        if (HttpGlobalConfig.getInstance().getHandlers() != null) {
            headres.putAll(HttpGlobalConfig.getInstance().getHandlers());
        }
    }
    ...
    private void addPramaers() {
        ...
        //添加公共的请求参数
        if (HttpGlobalConfig.getInstance().getParmes() != null) {
            paramser.putAll(HttpGlobalConfig.getInstance().getParmes());
        }
    }
    ...
    private Observable createObservable() {
        ...
        if(HttpGlobalConfig.getInstance().getBaseuri()!=null){
            this.baseUrl=HttpGlobalConfig.getInstance().getBaseuri();
        }
        ...
        ApiServer apiServer = HttpManager.getInstance().getRetorfit(baseUrl, time, timeUnit).create(ApiServer.class);
        switch (method) {
            case POST:
                if (isJson) {
